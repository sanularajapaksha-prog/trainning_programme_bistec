...
sequenceDiagram
  ...
  alt Receipt upload fails (network error or timeout)
    ...
    Note over FE,U: Client retries upload with fresh signed URLs; claim remains in Draft until receipts land
...
sequenceDiagram
  ...
  API-->>WRONGMGR: 403 Forbidden { code: "NOT_CLAIM_MANAGER", message: "Only the assigned line manager may approve this claim." }
